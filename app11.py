...
BUILD_ID = "MENU_MOVIL_DESLIZABLE_REAL_V23"
...
# Menú exclusivo para teléfonos: cinta horizontal deslizable.
# No usa columnas de Streamlit, por lo que los nombres nunca se comprimen.
mobile_menu_links = []
for mobile_option in MENU_OPTIONS:
    active_class = " active" if mobile_option == menu else ""
    aria_current = ' aria-current="page"' if mobile_option == menu else ""
    mobile_menu_links.append(
        f'<a class="mobile-scroll-item{active_class}" '
        f'href="?menu={mobile_option}" target="_self"{aria_current}>'
        f'{html.escape(mobile_option)}</a>'
    )

st.markdown(
    '<nav class="mobile-scroll-nav" aria-label="Menú principal">'
